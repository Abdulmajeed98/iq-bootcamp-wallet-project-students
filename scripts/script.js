...
class Wallet {
  ...
  static getSelectedWalletLocalStorage() {
    ...
    const transactions = walletInfo.transactions.map((e) => {
      if (e.type === "expense")
        return new Expense(e.amount, new Date(e.date), e.note, e.tags);
      if (e.type === "income")
        return new Income(e.amount, new Date(e.date), e.note, e.tags);
    });
  ...
  renderTransactions(ul) {
    ...
    this.transactions.forEach((transaction) =>
      ul.insertAdjacentHTML("beforeend", transaction.html())
    );
  }
}
...
class Transaction {
  ...
  html() {
    const badges = this.tags.reduce(
      (acc, tag) =>
        acc + ` <span class="badge badge-pill badge-dark">${tag}</span> `,
      ""
    );
...
function reloadLocalStorage() {
  ...
  if (selectedWallet != null) {
    ...
    currentMoney.innerText =
      selectedWallet.currency.symbol + " " + selectedWallet.balance;
    ...
    wallets.forEach((wallet) => {
      if (wallet.walletKey != selectedWallet.walletKey())
        dropdownMenu.insertAdjacentHTML(
          "beforeend",
          `<a class="dropdown-item" href="#" onclick="setSelectedWalletKeyLocalStorage('${wallet.walletKey}')">${wallet.name}</a>`
        );
    });
    dropdownMenu.insertAdjacentHTML(
      "beforeend",
    ...
    dropdownMenu.insertAdjacentHTML(
      "beforeend",
      ` <a class="dropdown-item" href="#" data-toggle="modal" data-target="#CreateWallet">Create wallet</a>`
    );
  } else {
